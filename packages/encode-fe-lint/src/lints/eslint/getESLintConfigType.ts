 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-mengtong/index
 * @returns eslint-config-mengtong/react
 * @returns eslint-config-mengtong/typescript/index
 * @returns eslint-config-mengtong/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'eslint-config-mengtong/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
