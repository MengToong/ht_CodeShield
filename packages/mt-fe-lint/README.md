...
`mt-fe-lint屏蔽了这些依赖和配置细节，提供简单的 CLI 和 Node.js API，让项目能够一键接入、一键扫描、一键修复、一键升级，并为项目配置 git commit 卡口，降低项目接入规范的成本。
...
```bash
npm install mt-fe-lint
```

安装完成后，可执行 `mt-fe-lint 以验证安装成功。
...
- 写入各种配置文件，包括：
  ...
  - `mt-fe-lint.config.js`mt-fmt-fe-lint如启用的功能等
...
>
> 注 2: `mt-fe-lint据项目内有无 eslint 和 stylelint 配置文件判断使用项目的配置文件还是 `mt-fmt-fe-lint扫描。若使用项目的，在未安装依赖时会帮其安装（执行 npm i）。若使用项目配置扫描失败，则使用默认配置扫描
...
- mt-fe-lint init(config)：将项目一键接入规范，效果等同于 `mt-fmt-fe-lint
...
`mt-fe-lint一份配置进行扫描（但你也可以零配置使用），支持的配置参数有：
